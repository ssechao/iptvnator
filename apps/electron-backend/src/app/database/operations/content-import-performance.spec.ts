...
describe('content import performance phases', () => {
    ...
    it('captures cache deletion as one pair across content groups and categories', async () => {
        const categoryRows = Array.from({ length: 150 }, (_, id) => ({
            id,
            xtreamId: id + 1000,
            hidden: id === 1,
        }));
        ...
        const insert = jest.fn(() => ({ values }));
        const db = { insert, select, transaction } as unknown as AppDatabase;
        const recording = createRecordingOperationPhaseCapture();
        ...
        expect(values).toHaveBeenCalledWith(
            expect.objectContaining({
                key: 'xtream-category-visibility:playlist-1:live',
                value: JSON.stringify({
                    version: 1,
                    hiddenXtreamIds: [1001],
                }),
            })
        );
        expect(recording.events).toEqual([
