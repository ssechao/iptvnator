...
async function markAutoRefreshDue(page: Page): Promise<void> {
    await page.evaluate(async (title) => {
        const playlists = await window.electron.dbGetAppPlaylistMetas();
        const playlist = playlists.find((item) => item.title === title);
        if (!playlist) {
            throw new Error(`Playlist not found: ${title}`);
        }

        const overdueTimestamp = Date.parse('2000-01-01T00:00:00.000Z');
        await window.electron.dbUpdatePlaylist(playlist._id, {
            updateDate: overdueTimestamp,
        });
...
test.describe('Electron startup playlist auto-refresh', () => {
    test('imports a protected URL with User-Agent and reuses it for manual and startup refresh', async ({
        ...
        try {
            ...
            await markAutoRefreshDue(app.mainWindow);
