...
export class VodDetailsSimilarService {
    ...
    readonly similarInPortals = computed<CrossPortalSimilarItem[]>(() => {
        ...
        const remaining = Math.max(
            0,
            SIMILAR_GRID_LIMIT - this.similarItems().length
        );
        return this.crossPortalItems()
            .filter(
                (item) =>
                    !localTitles.has(normalizeTitleKeys(item.title).exact)
            )
            .slice(0, remaining);
    });
