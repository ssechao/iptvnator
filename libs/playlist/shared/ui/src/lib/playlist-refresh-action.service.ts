...
export class PlaylistRefreshActionService {
    ...
    /** Refreshes a playlist immediately, for scheduled runs without a dialog. */
    async refreshNow(playlist: PlaylistMeta): Promise<boolean> {
        if (this.isRefreshing() || !this.canRefresh(playlist)) {
            return false;
        }

        if (playlist.serverUrl) {
            return this.xtreamRefreshFlow.refreshWithoutConfirmation(
                playlist,
        ...
        if (
            ...
            return this.refreshM3u(playlist);
        }
    ...
    private async refreshM3u(item: PlaylistMeta): Promise<boolean> {
